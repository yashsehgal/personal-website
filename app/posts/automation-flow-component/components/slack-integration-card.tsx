import { AutomationLogType, LOG_TYPE } from '@/app/posts/interfaces/main';
...
export function SlackAutomationCard({
  ...
  return (
    <div className="SlackAutomationCard-wrapper p-1 rounded-xl border relative border-neutral-200">
      <motion.div className="SlackAutomationCard relative bg-white drop-shadow rounded-lg p-4 flex items-center gap-3 overflow-hidden">
        {currentLog.logType === LOG_TYPE.SLACK_MESSAGE && (
          <motion.div
            key={currentLog.id}
            ref={scope}
            className="absolute top-4 w-[120px] h-[120px] border-[8px] border-purple-400 blur-[24px] border-neutral-200"
            initial={{ scale: 0 }}
          />
        )}
        <Image
