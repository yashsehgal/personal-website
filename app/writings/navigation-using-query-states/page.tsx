...
export default function BreadcrumbComponentPage() {
  return (
    <div className="mx-auto flex w-full min-w-0 max-w-3xl flex-col items-start gap-10 px-1 wide:mt-16">
      ...
      <div className="space-y-12">
        <h2 className="font-medium tracking-tight text-foreground">Breadcrumb component</h2>
        <p className={copyClassName}>
          ...
          <span className="text-foreground">&quot;Home&quot;</span> label and
      ...
      <div className="space-y-12">
        <h2 className="font-medium tracking-tight text-foreground">Making the navigation support backward compatibility</h2>
        <p className={copyClassName}>
        ...
        <p className={copyClassName}>
          ...
          query state (e.g.,{' '}
          <span className="text-foreground">engineering/setup/frontend</span>)
          ...
          <span className="text-foreground">engineering</span>, then{' '}
          <span className="text-foreground">engineering/setup</span>, and
          finally{' '}
          <span className="text-foreground">engineering/setup/frontend</span>.
        ...
        <p className={copyClassName}>
          ...
          <code className="font-mono text-foreground">DashboardSidebarComponent</code> works by computing required
        ...
        <pre className="code-block max-lg:hidden">
          <code className="font-mono text-foreground">{`const requiredExpandedNodes = useMemo(
  ...
  [selectedFileNodeId],
        ...
        <pre className="code-block">
          <code className="font-mono text-foreground">{`const expandedNodes = useMemo(() => {
          ...
          <>
            ...
            <motion.div
              ...
              <div className="px-5 py-2 border border-foreground/12 rounded-t-xl h-64 w-64 bg-background">
                ...
                <div className="h-8 flex items-center justify-start gap-2 pl-[50px] text-muted-foreground">
                  <IconPointFilled size={DASHBOARD_SIDEBAR_TREE_NODE_ICON} />
...
function DashboardSidebarComponent() {
  ...
  const expandedNodes = useMemo(() => {
    ...
    manuallyExpandedNodes.forEach((nodeId) => {
      if (!manuallyCollapsedNodes.has(nodeId)) {
        merged.add(nodeId);
      }
    });
  ...
  return (
    <aside className="h-full w-60 overflow-y-auto scroll-mask-y [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {DIRECTORY.map((node) => {
        return (
...
function DashboardSidebarNodeComponent(
  ...
  return (
    <button
      ...
      <p
        className={cn(
          'text-sm font-medium text-muted-foreground truncate group-hover/tree-node:text-foreground',
          isNodeSelected && 'text-foreground',
        )}>
        {node.name}
      </p>
