...
const ContactItem: React.FunctionComponent<ContactItemProps> = ({ title, link, className, ...attr }) => {
    return (
        <div className={cn("contact-item flex flex-row items-center justify-start gap-4", className)} {...attr}>
            <span className="contact-title font-normal text-sm">{title}</span>
            <span className="contact-link-content">
                {link?.url && <LinkText className="text-sm font-normal text-zinc-500" href={link?.url}>{link?.display}</LinkText>}
                {!link?.url && <span className="text-sm font-normal text-zinc-500">{"link not found"}</span>}
            </span>
        </div>
    )
}
